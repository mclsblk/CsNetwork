...
void handle_ip_packet(iface_info_t *iface, char *packet, int len)
{
	...
	// log(DEBUG, "IP packet: src %u, dst %u, iface %u", src_ip, dst_ip, iface_ip);
	...
	else if (ip->daddr == htonl(MOSPF_ALLSPFRouters)) { // 多播地址224.0.0.5
		assert(ip->protocol == IPPROTO_MOSPF);
		// log(DEBUG, "Received MOSPF packet destined to all SPF routers");
